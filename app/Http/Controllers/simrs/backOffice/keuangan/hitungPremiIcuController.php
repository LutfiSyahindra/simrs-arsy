...
use App\Services\keuangan\premi\generateIcuService;
use Illuminate\Http\Request;
use Yajra\DataTables\Facades\DataTables;

class hitungPremiIcuController extends Controller
{
    public function __construct(
        protected generateIcuService $service
    ) {}

    public function index()
    {
        return view('simrs.backOffice.keuangan.hitungPremi.generateIcu.generateIcu');
    }
    ...
    public function table(Request $request)
    {
        $validated = $request->validate([
            ...
            'jenis_icu' => ['nullable', 'in:umum,bpjs'],
        ]);
    ...
    public function summary(Request $request)
    {
        $validated = $request->validate([
            ...
            'jenis_icu' => ['required', 'in:umum,bpjs'],
        ]);
    ...
    public function config(Request $request)
    {
        ...
        return response()->json([
            'status' => true,
            'data' => $this->service->getConfig($validated['jenis_icu']),
        ]);
    }
    ...
    public function updateConfig(Request $request)
    {
        $validated = $request->validate([
            ...
            'perawat_icu_percent' => ['required', 'numeric', 'min:0', 'max:100'],
            'pegawai_icu_khusus_percent' => ['required', 'numeric', 'min:0', 'max:100'],
            'perawat_icu_reguler_percent' => ['required', 'numeric', 'min:0', 'max:100'],
            'perawat_icu_divider' => ['required', 'integer', 'min:1', 'max:999'],
            ...
            'recipients.perawat_icu' => ['nullable', 'array'],
            'recipients.perawat_icu.*' => ['string', 'max:30'],
            'recipients.pegawai_icu_khusus' => ['nullable', 'array'],
            'recipients.pegawai_icu_khusus.*' => ['string', 'max:30'],
        ]);

        return response()->json([
            'status' => true,
            'message' => 'Konfigurasi premi ICU berhasil disimpan.',
            'data' => $this->service->updateConfig($validated['jenis_icu'], $validated),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        return response()->json([
            'status' => true,
            'message' => "Premi ICU {$result['jenis_icu_label']} berhasil digenerate.",
            'data' => $result,
        ]);
    }
    ...
    public function lock(Request $request, int $id)
    {
        return response()->json([
            'status' => true,
            'message' => 'Data ICU berhasil dikunci.',
    ...
    public function unlock(Request $request, int $id)
    {
        return response()->json([
            'status' => true,
            'message' => 'Kunci data ICU berhasil dibuka.',
    ...
    public function destroy(int $id)
    {
        ...
        return response()->json([
            'status' => true,
            'message' => 'Data ICU berhasil dihapus.',
        ]);
    }
}
