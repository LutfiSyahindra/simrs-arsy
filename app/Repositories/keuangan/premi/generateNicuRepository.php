...
use App\Models\dbSimrs\generateNicuConfigModel;
use App\Models\dbSimrs\generateNicuModel;
...
class generateNicuRepository
{
    ...
    {
        $rows = $periode
            ? generateNicuModel::query()
                ->where('periode', $periode)
                ->where('jenis_nicu', $jenisIcu)
                ->get()
            : collect();
        ...
        return [
            ...
            'jumlah_pasien_nicu' => $rows->sum('jumlah_pasien_nicu'),
            'jumlah_pasien' => $rows->sum('jumlah_pasien'),
            'jumlah_tindakan' => $rows->sum('jumlah_tindakan'),
            'jumlah_tindakan_nicu' => $rows->sum('jumlah_tindakan_nicu'),
            'jumlah_tindakan_kritikal' => $rows->sum('jumlah_tindakan_kritikal'),
            'grand_total' => $rows->sum('grand_total'),
            'total_perawat_nicu' => $rows->sum('total_perawat_nicu'),
            'total_perawat_nicu_reguler' => $rows->sum('total_perawat_nicu_reguler'),
            'total_pegawai_nicu_khusus' => $rows->sum('total_pegawai_nicu_khusus'),
    ...
    public function getConfig(string $jenisIcu): generateNicuConfigModel
    {
        $this->ensureDefaultConfigs();

        $config = generateNicuConfigModel::query()
            ->with([
    ...
    public function saveConfig(
        ...
        return DB::transaction(function () use ($jenisIcu, $payload, $mappingIds, $recipients) {
            ...
            if ($mappingIds->isNotEmpty()) {
                DB::table('generate_nicu_config_tindakan')->insert(
                    $mappingIds
                        ->map(fn ($id) => [
                            'config_id' => $config->id,
                            'jnsTindakan_id' => $id,
            ...
            if (! empty($pegawaiRows)) {
                DB::table('generate_nicu_config_pegawai')->insert($pegawaiRows);
            }
    ...
    public function getSourceData(string $periode, string $jenisIcu, array $config): array
    {
        ...
        $nicuRanges = $admissions->isEmpty()
            ? collect()
            : $this->nicuRanges($admissions->keys()->values());
        ...
        if ($admissions->isNotEmpty() && $nicuRanges->isNotEmpty()) {
            foreach (self::SOURCE_TABLES as $definition) {
                ...
                foreach ($admissions->keys()->chunk(600) as $noRawatChunk) {
                    ...
                    foreach ($rows as $row) {
                        $patientIcuRanges = $nicuRanges->get($row->no_rawat, collect());
                        ...
                        $rangeMatch = $this->matchingNicuRange($row, $patientIcuRanges);
        ...
        return [
            ...
            'jumlah_pasien_nicu' => $nicuRanges->keys()->count(),
            'jumlah_pasien' => $details->pluck('no_rawat')->unique()->count(),
            'jumlah_tindakan' => $details->count(),
            'jumlah_tindakan_nicu' => $details->where('is_in_nicu_range', true)->count(),
    ...
    public function findExistingForUpdate(string $periode, string $jenisIcu): ?generateNicuModel
    {
        return generateNicuModel::query()
            ->where('periode', $periode)
            ->where('jenis_nicu', $jenisIcu)
    ...
    public function saveResult(string $periode, string $jenisIcu, array $calculation): generateNicuModel
    {
        ...
        $result = generateNicuModel::query()->updateOrCreate(
            [
                'periode' => $periode,
                'jenis_nicu' => $jenisIcu,
            ],
            [
                ...
                'jumlah_pasien_nicu' => $source['jumlah_pasien_nicu'],
                'jumlah_pasien' => $source['jumlah_pasien'],
                'jumlah_tindakan' => $source['jumlah_tindakan'],
                'jumlah_tindakan_nicu' => $source['jumlah_tindakan_nicu'],
                'jumlah_tindakan_kritikal' => $source['jumlah_tindakan_kritikal'],
                'grand_total' => $source['grand_total'],
                'total_perawat_nicu' => $pools['perawat_nicu'],
                'total_perawat_nicu_reguler' => $pools['perawat_nicu_reguler'],
                'total_pegawai_nicu_khusus' => $pools['pegawai_nicu_khusus'],
        ...
        $source['details']
            ->map(fn (array $detail) => [
                ...$detail,
                'generate_nicu_id' => $result->id,
                'created_at' => $now,
                'updated_at' => $now,
            ])
            ->chunk(500)
            ->each(fn (Collection $chunk) => DB::table('generate_nicu_detail')->insert($chunk->all()));
        ...
        collect($calculation['recipients'])
            ...
            ->chunk(200)
            ->each(fn (Collection $chunk) => DB::table('generate_nicu_recipient')->insert($chunk->all()));

        return $result->fresh(['details', 'recipients', 'jenisTindakan:id,kode,jenis', 'lockedBy:id,name', 'generateBy:id,name']);
    }

    public function findForUpdate(int $id): ?generateNicuModel
    {
        return generateNicuModel::query()
            ->lockForUpdate()
            ->find($id);
    ...
    public function findWithDetails(int $id): ?generateNicuModel
    {
        return generateNicuModel::query()
            ->with([
                'jenisTindakan:id,kode,jenis',
                ...
                'recipients' => fn ($query) => $query
                    ->orderByRaw("FIELD(role, 'perawat_nicu', 'pegawai_nicu_khusus')")
    ...
    public function updateLock(
        generateNicuModel $result,
        bool $isLocked,
        ?int $userId = null
    ): generateNicuModel {
        DB::table('generate_nicu')
            ->where('id', $result->id)
            ->update([
                'is_locked' => $isLocked,
    ...
    public function deleteResult(generateNicuModel $result): void
    {
        $result->delete();
    }
    ...
    private function matchingNicuRange(object $row, Collection $ranges): ?object
    {
    ...
    private function detailPayload(
        ...
        return [
            ...
            'kd_kamar_nicu' => $icuRange->kd_kamar,
            'tgl_masuk_nicu' => $this->cleanDate($icuRange->tgl_masuk),
            'jam_masuk_nicu' => $this->cleanTime($icuRange->jam_masuk),
            'tgl_keluar_nicu' => $this->cleanDate($icuRange->tgl_keluar),
            'jam_keluar_nicu' => $this->cleanTime($icuRange->jam_keluar),
            'tanggal' => $row->tanggal,
    ...
    private function ensureDefaultConfigs(): void
    {
        foreach (['umum', 'bpjs'] as $jenis) {
            generateNicuConfigModel::query()->firstOrCreate(
                ['jenis_nicu' => $jenis],
                [
                    'jnsTindakan_id' => null,
                    'perawat_nicu_percent' => 30,
                    'pegawai_nicu_khusus_percent' => 25,
                    'perawat_nicu_reguler_percent' => 75,
                    'perawat_nicu_divider' => 4,
                    'premi_medis_percent' => 25,
                    'premi_medis_divider' => 34,
                    'premi_bersama_percent' => 15,
                    'critical_action_name' => null,
    ...
    private function ensureConfigMappings(generateNicuConfigModel $config): void
    {
