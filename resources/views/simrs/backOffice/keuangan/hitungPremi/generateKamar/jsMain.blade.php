<script>
    $(function() {
        ...
        function loadConfig(jenis) {
            ...
            configRequest = $.ajax({
                url: "{{ route("backOffice.keuangan.hitungPremi.generateKamar.config") }}",
                data: { jenis_kamar: jenis },
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#summaryFormulaBhp').text(data.nominal_is_mixed ?
                        (data.jumlah_lama_inap || 0) +
                        ' hari x nominal masing-masing ploting' :
                ...
                error: function() {
                    ...
                    $('#summaryPeriode').text('-');
                    $('#summaryFormulaBhp').text('0 hari x Rp 0 x 0 ploting');
                    updateLockState({
                        is_locked: false,
                        jumlah_kamar: 0,
        ...
        function updateLockState(data) {
            ...
            lockAllButton.attr(
                'title',
                unlockedPloting > 0 ?
                'Kunci ' + unlockedPloting + ' data kamar yang masih terbuka.' :
                (hasResult ? 'Semua data kamar sudah terkunci.' : 'Belum ada data kamar yang bisa dikunci.')
            );
        ...
        $('#btnLockAllBhp').on('click', function() {
            Swal.fire({
                icon: 'warning',
                title: 'Kunci semua data kamar?',
                text: 'Semua data kamar ' + typeConfig[activeType].label + ' periode ' +
                ...
                $.ajax({
                    url: "{{ route("backOffice.keuangan.hitungPremi.generateKamar.lockAll") }}",
                    type: 'POST',
                    data: {
                        periode: $('#periodeBhp').val(),
                        jenis_kamar: activeType
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            title: 'Data Dikunci',
                            text: response.message ||
                                'Semua data kamar berhasil dikunci.',
                            timer: 1600,
                            showConfirmButton: false
                        });
                    },
        ...
        $('#tableGenerateBhp').on('click', '.btn-delete-kamar', function() {
            const id = $(this).data('id');
            ...
            Swal.fire({
                ...
                $.ajax({
                    url: "{{ route("backOffice.keuangan.hitungPremi.generateKamar.delete", ["id" => "__ID__"]) }}"
                        .replace('__ID__', id),
                    type: 'DELETE',
                    success: function(response) {
                        refreshBhpData();
            ...
            loadConfig(activeType)
                ...
                .fail(function(xhr) {
                    if (xhr.statusText !== 'abort') {
                        Swal.fire('Gagal', getErrorMessage(xhr, 'Konfigurasi kamar gagal dimuat.'), 'error');
                    }
                });
        });
        ...
        $('#btnConfigBhp').on('click', function() {
            loadConfig(activeType)
                .done(function() {
                    $('#modalConfigBhpLabel').text('Konfigurasi Kamar ' + typeConfig[activeType].label);
        ...
        $('#formConfigBhp').on('submit', function(event) {
            ...
            $.ajax({
                url: "{{ route("backOffice.keuangan.hitungPremi.generateKamar.updateConfig") }}",
                type: 'PUT',
                data: {
                    jenis_kamar: jenis,
                ...
                success: function(response) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Berhasil',
                        text: response.message || 'Konfigurasi kamar berhasil disimpan.',
                        timer: 1600,
                        showConfirmButton: false
                    });
                },
                error: function(xhr) {
                    Swal.fire('Gagal', getErrorMessage(xhr, 'Konfigurasi kamar gagal disimpan.'), 'error');
                },
                complete: function() {
