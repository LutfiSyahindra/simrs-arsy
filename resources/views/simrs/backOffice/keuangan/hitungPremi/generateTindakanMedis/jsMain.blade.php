<script>
    $(function() {
        ...
        function renderDistributionRows(selector, distributions) {
            const tbody = $(selector).empty();
            distributions = distributions || [];

            if (!distributions.length) {
                tbody.html('<tr><td colspan="5" class="tm-empty">Belum ada pegawai penerima pada mapping premi.</td></tr>');
                return;
            }
            ...
            distributions.forEach(function(item) {
                tbody.append(`
                    <tr>
                        ...
                        <td class="text-end fw-bold">${formatRupiah(item.total_diterima || 0)}</td>
                    </tr>
                `);
            });
        }
        ...
        function renderSummary(data) {
            ...
            $('#sourceUgdTindakanMedisNote').text(
                selectedUgdSourceId ? 'Sumber UGD terpilih.' : 'Pilih hasil UGD sesuai ploting.'
            );
            $('#sourceVkTindakanMedisNote').text(
                selectedVkSourceId ? 'Sumber VK terpilih.' : 'Pilih hasil VK sesuai ploting.'
            );
            ...
            $('#summaryTindakanMedisSubtitle').text(
                data.readiness_message + ' Periode sumber: ' + (data.source_periode || '-') + karcisText
            );
            ...
            $('#summaryDistributionCountMedis').text(formatNumber(data.jumlah_penerima || 0) + ' penerima');
            ...
            renderDistributionRows('#summaryDistributionRowsMedis', data.distributions || []);
        ...
        function renderPreviewDetails(details) {
            ...
            $('#summaryPreviewCountMedis').text(formatNumber(details.length) + ' tindakan');
