<script>
    $(function() {
        ...
        function renderSummarySpotlight(data) {
            ...
            const generated = Number(data.generated_count || 0);
            const locked = Number(data.locked_count || 0);
            const actionMix = formatNumber(data.jumlah_tindakan_icu) + ' ICU / ' +
            ...
            $('#summaryIcuSpotlight').html(
                ...
                summaryMetric('Pasien ICU', formatNumber(data.jumlah_pasien_icu), 'Dari ' + formatNumber(data
        ...
        function renderSummaryInsights(data) {
            ...
            $('#summaryIcuInsights').html([
                summaryInsight('Cakupan ICU', percentText(percentOf(data.jumlah_pasien_icu, data
                        .jumlah_pasien_sumber)),
                    formatNumber(data.jumlah_pasien_icu) + ' dari ' + formatNumber(data
                        .jumlah_pasien_sumber) + ' pasien sumber.', 'mdi-account-injury-outline'),
                summaryInsight('Tindakan Terkait ICU', formatNumber(data.jumlah_tindakan_icu),
                    formatNumber(data.jumlah_tindakan_kritikal) + ' tindakan kritikal ikut terhitung.',
                    'mdi-clipboard-pulse-outline'),
                summaryInsight('Pool ICU Reguler', formatRupiah(data.total_perawat_icu_reguler),
                    'Bagian reguler dari pool Perawat ICU.', 'mdi-account-heart-outline'),
        ...
        function renderSummaryDistribution(data) {
            ...
            $('#summaryIcuDistribution').html(
                ...
                '<div class="icu-summary-bars">' + [
                    summaryBar('Pool Perawat ICU', data.total_perawat_icu, total,
                        'basis utama ICU dari konfigurasi', ''),
                    summaryBar('ICU Reguler', data.total_perawat_icu_reguler, total,
                        'porsi reguler sesuai konfigurasi', 'teal', data.total_perawat_icu,
                        'Pool Perawat ICU'),
                    summaryBar('ICU Khusus', data.total_pegawai_icu_khusus, total,
                        'sub-pool pegawai khusus sesuai konfigurasi', 'amber', data.total_perawat_icu,
                        'Pool Perawat ICU'),
        ...
        function formulaBarsHtml(config) {
            config = config || {};
            const khusus = clampPercent(config.pegawai_icu_khusus_percent || 0);
            const reguler = clampPercent(config.perawat_icu_reguler_percent != null ?
                config.perawat_icu_reguler_percent : Math.max(0, 100 - khusus));
            return '<div class="icu-bar-list">' + [
                barHtml('Pool Perawat ICU', config.perawat_icu_percent, 'Porsi dari grand total tindakan ICU.',
                    ''),
                barHtml('ICU Khusus', khusus, 'Porsi dari pool Perawat ICU.', 'amber'),
                barHtml('ICU Reguler', reguler, 'Porsi manual dari pool Perawat ICU.',
                    'teal'),
        ...
        function formulaRows(data) {
            ...
            const regulerPercent = config.perawat_icu_reguler_percent != null ?
                Number(config.perawat_icu_reguler_percent) :
                Math.max(0, 100 - Number(config.pegawai_icu_khusus_percent || 25));
            ...
            return [{
                ...
                {
                    label: 'Perawat ICU',
                    value: percentText(config.perawat_icu_percent),
                    note: 'Pool utama dari grand total tindakan ICU.'
                },
                {
                    label: 'Pegawai ICU Khusus',
                    value: percentText(config.pegawai_icu_khusus_percent),
                    note: 'Diambil dari pool Perawat ICU.'
                },
                {
                    label: 'Perawat ICU Reguler',
                    value: percentText(regulerPercent) + ' / ' + formatNumber(config.perawat_icu_divider ||
                        4),
                    note: 'Porsi manual dari pool ICU, dibagi sesuai pembagi dan pegawai terpilih.'
                },
                {
                    label: 'Premi Medis',
                ...
                },
                {
                    ...
                    note: 'Diambil dari grand total tindakan ICU.'
                },
                {
                ...
                {
                    label: 'Penerima Perawat ICU',
                    value: formatNumber((config.recipient_counts || {}).perawat_icu || (config.recipients &&
                            config.recipients.perawat_icu ? config.recipients.perawat_icu.length : 0)) +
                        ' pegawai',
                    note: 'Jumlah penerima harus sama dengan pembagi pool reguler.'
                },
                {
                    label: 'Penerima ICU Khusus',
                    value: formatNumber((config.recipient_counts || {}).pegawai_icu_khusus || (config
                        .recipients && config.recipients.pegawai_icu_khusus ? config.recipients
                        .pegawai_icu_khusus.length : 0)) + ' pegawai',
                    note: 'Menerima sub-pool pegawai ICU khusus.'
                }
            ];
        }
        ...
        function renderSummary(data) {
            $('#summaryIcuGrid').html([
                ...
                miniCard('Pasien ICU', formatNumber(data.jumlah_pasien_icu), false, 'mdi-bed-outline',
                    'Riwayat kd_kamar memuat ICU', 'data'),
                miniCard('Tindakan', formatNumber(data.jumlah_tindakan), false,
                    'mdi-clipboard-pulse-outline', 'Mapping dan tindakan kritikal', 'data'),
                miniCard('Pool ICU', formatRupiah(data.total_perawat_icu), false,
                    'mdi-account-heart-outline', 'Khusus dan reguler sesuai konfigurasi', 'distribusi'),
                miniCard('ICU Reguler', formatRupiah(data.total_perawat_icu_reguler), false,
                    'mdi-account-multiple-check-outline', 'Pool reguler sebelum dibagi', 'distribusi'),
                miniCard('Pegawai Khusus', formatRupiah(data.total_pegawai_icu_khusus), false,
                    'mdi-account-star-outline', 'Porsi khusus dari pool ICU', 'distribusi'),
        ...
        function applySummaryView() {
            ...
            const noteMap = {
                ...
                distribusi: 'Fokus pada pembagian pool ICU.'
            };
        ...
        function loadSummary() {
            ...
            $.get("{{ route("backOffice.keuangan.hitungPremi.generateIcu.summary") }}", {
                periode: $('#periodeIcu').val(),
                jenis_icu: activeType
        ...
        function initTable() {
            tableIcu = $('#tableGenerateIcu').DataTable({
                ...
                ajax: {
                    url: "{{ route("backOffice.keuangan.hitungPremi.generateIcu.table") }}",
                    data: function(d) {
                        d.periode = $('#periodeIcu').val();
                        d.jenis_icu = activeType;
                    }
                },
                columns: [{
                        data: 'DT_RowIndex',
                    ...
                    },
                    {
                        ...
                        render: function(row) {
                            return '<strong>' + formatNumber(row.jumlah_tindakan) +
                                '</strong>' +
                                '<div class="text-muted small">ICU ' + formatNumber(row
                                    .jumlah_tindakan_icu) +
        ...
        function initConfigSelects() {
            $('#configIcuMapping').select2({
                ...
                width: '100%',
                ajax: {
                    url: "{{ route("backOffice.keuangan.hitungPremi.generateIcu.mappingOptions") }}",
                    dataType: 'json',
                    delay: 250,
            ...
            $('#configCriticalActionName').select2({
                ...
                width: '100%',
                ajax: {
                    url: "{{ route("backOffice.keuangan.hitungPremi.generateIcu.criticalActionOptions") }}",
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
                        return {
            ...
            $('#configPerawatIcuRecipients, #configPegawaiIcuKhususRecipients').select2({
                ...
                width: '100%',
                ajax: {
                    url: "{{ route("backOffice.keuangan.hitungPremi.generateIcu.pegawaiOptions") }}",
                    dataType: 'json',
                    delay: 250,
                    data: function(params) {
                        return {
        ...
        function updateRecipientCounts() {
            ...
            khususHelp
                ...
                .text(
                    formatNumber(khususCount) + ' penerima dipilih untuk pool khusus ICU.'
                );
        }
        ...
        function configFromFields() {
            ...
            return {
                ...
                perawat_icu_percent: Number($('#configPerawatIcuPercent').val() || 0),
                pegawai_icu_khusus_percent: Number($('#configPegawaiIcuKhususPercent').val() || 0),
                perawat_icu_reguler_percent: Number($('#configPerawatIcuRegulerPercent').val() || 0),
                perawat_icu_divider: Math.max(1, Number($('#configPerawatIcuDivider').val() || 4)),
                ...
                recipient_counts: {
                    perawat_icu: ($('#configPerawatIcuRecipients').val() || []).length,
                    pegawai_icu_khusus: ($('#configPegawaiIcuKhususRecipients').val() || []).length
                }
            };
        }
        ...
        function renderConfigFormula() {
            ...
            const perawatCount = (config.recipient_counts || {}).perawat_icu || 0;
            const khususCount = (config.recipient_counts || {}).pegawai_icu_khusus || 0;
            const icuDistributionPercent = Number(config.pegawai_icu_khusus_percent || 0) +
                Number(config.perawat_icu_reguler_percent || 0);
            ...
            $('#configIcuLiveSummary').text(
                percentText(config.perawat_icu_percent) + ' pool, khusus ' +
                percentText(config.pegawai_icu_khusus_percent) + ', reguler ' +
                percentText(config.perawat_icu_reguler_percent) + ' / ' + formatNumber(config
                    .perawat_icu_divider)
            );
            $('#configRecipientHealth').html(infoRowsHtml([{
                ...
                },
                {
                    ...
                    note: criticalCount > 0 ? config.critical_action_label :
                        'Opsional, tetapi membantu menangkap tindakan khusus ICU.'
                },
                {
                    label: 'Perawat ICU reguler',
                    value: formatNumber(perawatCount) + ' pegawai',
                    note: 'Porsi ' + percentText(config.perawat_icu_reguler_percent) +
                        ', pembagi aktif: ' + formatNumber(config.perawat_icu_divider) + '.'
                },
                {
                    label: 'Pegawai ICU khusus',
                    value: formatNumber(khususCount) + ' pegawai',
                    note: khususCount > 0 ? 'Porsi ' + percentText(config.pegawai_icu_khusus_percent) +
                        ' dari pool ICU.' :
                        'Wajib diisi jika pool khusus bernilai.'
                },
                {
                    label: 'Total distribusi ICU',
                    value: percentText(icuDistributionPercent),
                    note: isDistributionValid ? 'Masih dalam batas pool Perawat ICU.' :
                        'Tidak boleh melebihi 100% dari pool Perawat ICU.'
                }
            ]));
            ...
            stateBanner(
                ...
                isReady ? 'Konfigurasi terlihat siap dipakai' : (isDistributionValid ?
                    'Konfigurasi perlu dilengkapi' : 'Distribusi ICU melebihi 100%'),
                isReady ?
                'Mapping dan penerima wajib sudah dipilih untuk jenis ICU ' + typeConfig[activeType] + '.' :
                (isDistributionValid ?
                    'Pastikan mapping tindakan, perawat ICU, dan pegawai ICU khusus sudah dipilih sebelum preview.' :
                    'Kurangi persentase Pegawai ICU Khusus atau Perawat ICU Reguler.'),
        ...
        function openConfig() {
            ...
            $('#configIcuTitle').text('Konfigurasi ICU ' + typeConfig[activeType]);
            $('#configIcuRuleCopy').text(sourceRuleText());
            $('#btnSubmitConfigIcu').prop('disabled', true);
            configModal.show();

            $.get("{{ route("backOffice.keuangan.hitungPremi.generateIcu.config") }}", {
                jenis_icu: activeType
            }).done(function(response) {
                const data = response.data || {};
                $('#configPerawatIcuPercent').val(data.perawat_icu_percent ?? 30);
                $('#configPegawaiIcuKhususPercent').val(data.pegawai_icu_khusus_percent ?? 25);
                $('#configPerawatIcuRegulerPercent').val(data.perawat_icu_reguler_percent ?? 75);
                $('#configPerawatIcuDivider').val(data.perawat_icu_divider ?? 4);
                ...
                setSelectOptions('#configPerawatIcuRecipients', recipients.perawat_icu || [],
                    'pegawai_id', 'text');
                setSelectOptions('#configPegawaiIcuKhususRecipients', recipients.pegawai_icu_khusus ||
                [], 'pegawai_id', 'text');
        ...
        function detailOverviewHtml(data) {
            ...
            return [
                ...
                auditTile('Jenis ICU', data.jenis_icu_label, 'Aturan penjamin mengikuti jenis generate.',
                ...
                auditTile('Pasien Sumber', formatNumber(data.jumlah_pasien_sumber), formatNumber(data
                    .jumlah_pasien_icu) + ' pasien punya riwayat ICU.', 'mdi-account-injury-outline'),
                auditTile('Tindakan Tersimpan', formatNumber(data.jumlah_tindakan), formatNumber(data
                        .jumlah_tindakan_icu) + ' range ICU, ' + formatNumber(data
        ...
        function detailFormulaRows(data) {
            ...
            const khususPercent = Number(config.pegawai_icu_khusus_percent || 25);
            const regulerPercent = config.perawat_icu_reguler_percent != null ?
                Number(config.perawat_icu_reguler_percent) :
                (pools.perawat_icu_reguler_percent != null ? Number(pools.perawat_icu_reguler_percent) :
                    Math.max(0, 100 - khususPercent));

            return [{
                    label: 'Pool Perawat ICU',
                    value: formatRupiah(pools.perawat_icu),
                    note: percentText(config.perawat_icu_percent) + ' dari grand total.'
                },
                {
                    label: 'ICU Khusus',
                    value: formatRupiah(pools.pegawai_icu_khusus),
                    note: percentText(khususPercent) + ' dari pool ICU.'
                },
                {
                    label: 'ICU Reguler',
                    value: formatRupiah(pools.perawat_icu_reguler),
                    note: percentText(regulerPercent) + ' / ' + formatNumber(pools.perawat_icu_divider ||
                        config.perawat_icu_divider || 4) + ' pembagi.'
                },
                {
        ...
        function detailRowsHtml(details, limit) {
            ...
            return '<table class="table table-sm table-hover align-middle">' +
                '<thead><tr>' +
                '<th>Waktu</th><th>Pasien</th><th>Tindakan</th><th>ICU</th><th>Pelaksana</th><th class="text-end">Biaya</th><th>Status</th>' +
                '</tr></thead><tbody>' +
                visible.map(function(item) {
                    ...
                    const icuRange = (item.tgl_masuk_icu || '-') + ' ' + (item.jam_masuk_icu || '') +
                        ' s/d ' + (item.tgl_keluar_icu || 'aktif') + ' ' + (item.jam_keluar_icu || '');
                    const status = [
                        item.is_in_icu_range ? chipHtml('Range ICU', 'success',
                    ...
                    return '<tr class="icu-detail-row">' +
                        ...
                        '</div></td>' +
                        '<td><div class="icu-cell-title">' + escapeHtml(item.kd_kamar_icu || '-') +
            ...
            return '<table class="table table-sm table-hover align-middle">' +
                ...
                recipients.map(function(item) {
                    return '<tr>' +
                        ...
                        '</div></td>' +
                        '<td>' + chipHtml(item.role_label || item.role || '-', item.role === 'perawat_icu' ?
                            'success' : 'warning', item.role === 'perawat_icu' ?
        ...
        function recipientGroupsHtml(groups) {
            ...
            if (!groups.length) {
                return emptyStateHtml('Belum ada penerima', 'Pilih penerima premi pada konfigurasi ICU.',
            ...
            return infoRowsHtml(groups.map(function(group) {
                ...
                const extra = (group.items || []).length > 4 ? ' +' + ((group.items || []).length -
                    4) : '';
                const perPersonNote = group.role === 'perawat_icu' && firstItem.total_received !=
                    null ?
        ...
        function renderPreview(data) {
            ...
            $('#generateIcuTitle').text('Preview ICU ' + (data.jenis_icu_label || typeConfig[activeType]));
            $('#generateIcuText').text('Periode generate ' + data.periode + ' memakai data sumber ' + (source
                .source_periode || '-'));
            $('#generateIcuTypeBadge').text(data.jenis_icu_label || typeConfig[activeType]);
            stateBanner(
            ...
            $('#previewIcuInsights').html([
                ...
                insightCard('Rasio Pasien ICU', percentText(percentOf(source.jumlah_pasien_icu, source
                        .jumlah_pasien_sumber)),
                    formatNumber(source.jumlah_pasien_icu) + ' dari ' + formatNumber(source
                        .jumlah_pasien_sumber) + ' pasien sumber.', 'mdi-bed-outline'),
                insightCard('Komposisi Tindakan', formatNumber(source.jumlah_tindakan_icu) + ' ICU / ' +
            ...
            $('#previewIcuStats').html([
                ...
                miniCard('Pasien ICU', formatNumber(source.jumlah_pasien_icu), false, 'mdi-bed-outline',
                    'kd_kamar memuat ICU'),
                miniCard('Tindakan', formatNumber(source.jumlah_tindakan), false,
                    'mdi-clipboard-pulse-outline', 'Sesuai mapping dan kritikal'),
                miniCard('Pool ICU', formatRupiah(pools.perawat_icu), false,
                    'mdi-account-heart-outline', 'Reguler ' + formatRupiah(pools.perawat_icu_reguler)),
                miniCard('Perawat Reguler', formatRupiah(pools.perawat_icu_per_orang), false,
                    'mdi-account-multiple-check-outline', 'Reguler / ' + formatNumber(pools
                        .perawat_icu_divider || config.perawat_icu_divider || 4)),
                miniCard('Pegawai Khusus', formatRupiah(pools.pegawai_icu_khusus), false,
                    'mdi-account-star-outline', percentText(config.pegawai_icu_khusus_percent) +
                    ' dari pool ICU'),
        ...
        function openPreview() {
            ...
            $.get("{{ route("backOffice.keuangan.hitungPremi.generateIcu.preview") }}", {
                periode: $('#periodeIcu').val(),
                jenis_icu: activeType
            }).done(function(response) {
        ...
        function renderDetail(data) {
            ...
            $('#detailIcuTitle').text('ICU ' + data.jenis_icu_label + ' - ' + data.periode);
            ...
            $('#detailIcuKpiGrid').html([
                ...
                detailKpi('Pasien ICU', formatNumber(data.jumlah_pasien_icu), percentText(percentOf(data
                    .jumlah_pasien_icu, data.jumlah_pasien_sumber)) + ' dari pasien sumber',
                    'mdi-bed-outline', false),
                detailKpi('Pool ICU', formatRupiah(pools.perawat_icu), 'Reguler ' + formatRupiah(pools
                    .perawat_icu_reguler), 'mdi-account-heart-outline', false),
        ...
        function openDetail(id) {
            ...
            const url = "{{ route("backOffice.keuangan.hitungPremi.generateIcu.detail", ["id" => "__ID__"]) }}"
                .replace('__ID__', id);
        ...
        function toggleLock(id, locked) {
            const url = (locked ?
                "{{ route("backOffice.keuangan.hitungPremi.generateIcu.lock", ["id" => "__ID__"]) }}" :
                "{{ route("backOffice.keuangan.hitungPremi.generateIcu.unlock", ["id" => "__ID__"]) }}"
            ).replace('__ID__', id);

            Swal.fire({
                title: locked ? 'Kunci data ICU?' : 'Buka kunci data ICU?',
        ...
        function deleteResult(id) {
            const url = "{{ route("backOffice.keuangan.hitungPremi.generateIcu.delete", ["id" => "__ID__"]) }}"
                .replace('__ID__', id);

            Swal.fire({
                title: 'Hapus data ICU?',
                ...
                $.ajax({
                    ...
                    Swal.fire('Berhasil', response.message || 'Data ICU dihapus.', 'success');
                    reloadTable();
                    loadSummary();
                }).fail(function(xhr) {
                    Swal.fire('Gagal', errorMessage(xhr), 'error');
        ...
        $('#formConfigIcu').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route("backOffice.keuangan.hitungPremi.generateIcu.updateConfig") }}",
                method: 'PUT',
                data: {
                    jenis_icu: activeType,
                    jnsTindakan_ids: $('#configIcuMapping').val() || [],
                    perawat_icu_percent: $('#configPerawatIcuPercent').val(),
                    pegawai_icu_khusus_percent: $('#configPegawaiIcuKhususPercent').val(),
                    perawat_icu_reguler_percent: $('#configPerawatIcuRegulerPercent').val(),
                    perawat_icu_divider: $('#configPerawatIcuDivider').val(),
                    ...
                    recipients: {
                        perawat_icu: $('#configPerawatIcuRecipients').val() || [],
                        pegawai_icu_khusus: $('#configPegawaiIcuKhususRecipients').val() || []
                    }
                }
            }).done(function(response) {
                Swal.fire('Berhasil', response.message || 'Konfigurasi ICU disimpan.',
                    'success');
                configModal.hide();
            }).fail(function(xhr) {
                Swal.fire('Gagal', errorMessage(xhr), 'error');
        ...
        $('#formGenerateIcu').on('submit', function(e) {
            ...
            $.post("{{ route("backOffice.keuangan.hitungPremi.generateIcu.store") }}", {
                periode: $('#periodeIcu').val(),
                jenis_icu: activeType
            }).done(function(response) {
                Swal.fire('Berhasil', response.message || 'Premi ICU berhasil digenerate.',
                    'success');
                generateModal.hide();
                reloadTable();
                loadSummary();
